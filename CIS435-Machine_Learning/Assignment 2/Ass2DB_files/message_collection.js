...
(function()
{
  ...
  var MessageTags;
  /**
   * The tagging behavior for the message_collection page
   * - Sets up the tag behavior for each message
   * - Sets up the action bar tagging multiple messages action.
   */
  var Tagging;
  /**
  ...
  Tagging.prototype = {
    /**
     * Constructor
     * @param messageStrings a hash of key to localized message strings used by this object
     *         required: removeConfirmMsg, addTagEmptyMsg, addTagSelectMessageMsg, addTagAlreadyTaggedMsg
    ...
    updateTagFilterSelectBox: function()
    {
      ...
      } catch (ex ) { // IE is not standards compliant
    ...
    onAddTagToMessages: function( event, tagTextBox )
    {
      ...
      {
        alert(JS_RESOURCES.getFormattedString('validation.maximum_length.no_name.singular',
                                              ['255']));
        return false;
      }
      else if ( tagValue.length > 255 )
      {
        alert(JS_RESOURCES.getFormattedString('validation.maximum_length.no_name.plural',
                                              ['255', tagValue.length - 255]));
        return false;
      }
  ...
  MessageTags.prototype = {
    ...
    onRemoveTag: function( tagValue )
    {
      ...
      MessageTaggingService.removeTagFromMessage( this.msgTagging.courseId, this.msgTagging.forumId, this.messageId, tagValue );
      return true;
    },
    ...
    onAddTag: function( tagValue )
    {
      if( this.msgTagging.tagToNumHash[ tagValue ] )
      {
        ...
        this.msgTagging.tagNameSet.push( tagValue );
        this.msgTagging.updateTagFilterSelectBox();
      }

      MessageTaggingService.addTagToMessage( this.msgTagging.courseId, this.msgTagging.forumId, this.messageId, tagValue );
    }
  };
  ...
  {
    new SelectAllCheckbox( containerId );
  };
