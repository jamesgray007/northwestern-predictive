  ...
  collab_util = {
    ...
    hideDesc: function(msg){
      ...
      if(msgspan.style.display=="" || msgspan.style.display=="inline"){
        applyState("none", "/images/ci/ng/more_options.gif", page.bundle.getString('collab.expand.instructions'));

        UserDataDWRFacade.setStringPermScope( "collab.instructionsExpanded", false );
      }else{
        applyState("inline", "/images/ci/ng/less_options_dark.gif", page.bundle.getString('collab.collapse.instructions'));
    ...
    lbSaveAndCloseComments: function()
    {
        ...
        var gradeElement = $('collabGradePoints');
        if (!collab_util.validGradeEntry(gradeElement))
        {
          return false;
        }

        var url ='/webapps/blackboard/execute/editCollabGrade?action=save';
        if ( window.gradeVersion )
        {
          url = url + '&grade_version=' + window.gradeVersion;
        }

        document.forms.EditCollabGradeForm.lastViewedTime.value = $('blogGradingForm').lastViewedTime.value;

        // NOTE that we explicitly want this to be a 'synchronous' call here because we want to
        // force the user to finish their interaction in the lightbox - i.e. if there is an error
        // then the lightbox stays open and they can try again or cut/paste into another application
        // and restart their browser, etc. - if it was asynchronous then they could click 'cancel' to
        // get rid of the lightbox (possibly in the misguided belief that they saved and it is taking too
        // long) and then they may lose their comments.
        var myAjax = new Ajax.Request( url,
                {
                  method: 'post',
                  parameters: Form.serialize(document.forms.EditCollabGradeForm),
                  onSuccess: collab_util.lbSaveGradeCommentsSuccess,
                  onFailure: collab_util.lbSaveGradeCommentsFailure,
                  onException: collab_util.lbSaveGradeCommentsException,
                  asynchronous: false
                });
    },

    lbSaveGradeCommentsSuccess: function(reply)
    {
      lightbox.closeCurrentLightbox();
      onGradeInfoSaved(reply);
    ...
    lbSaveGradeCommentsFailure: function(reply)
    {
      var result = reply.responseText.evalJSON();
      document.forms.EditCollabGradeForm.elements['blackboard.platform.security.NonceUtil.nonce'].value = result.nonce;
      alert(result.message);
    },

    lbSaveGradeCommentsException: function(reply)
    {
      alert(page.bundle.getString('collab.comments.vtbe.save.error'));
    },
    ...
    {
      // Open a lightbox
      var gradingForm = $('blogGradingForm');
      var userId = gradingForm.course_user_id.value;
      var linkIdRef = gradingForm.link_id_ref.value;
      var courseId = gradingForm.course_id.value;
      var groupId = gradingForm.group_id.value;
      var groupAttemptId = gradingForm.groupAttemptId.value;
      var url = '/webapps/blackboard/execute/editCollabGrade?course_id='+courseId+
            '&courseUserId='+ userId +
            '&linkIdRef=' + linkIdRef +
            '&groupId=' + groupId +
            '&groupAttemptId=' + groupAttemptId;

      var lightboxParam = {
              defaultDimensions: { w:800, h:600 },
              ajax: {url: url,
                loadExternalScripts: true},
              title: page.bundle.getString('collab.comments.vtbe.title'),
              closeOnBodyClick: false,
              showCloseLink: false,
              contents: '',
              focusOnClose: 'gradeBlogLink',
              useDefaultDimensionsAsMinimumSize: true
          };

        var gradeLightbox = new lightbox.Lightbox( lightboxParam );
        gradeLightbox.open( );
        showGradeInfo();
    ...
    conditionalVtbe: function( msg )
    {
      // Leela, Stephanie, and Ko want to skip this confirmation and go direct to the vtbe..
      // or at least they want to try it out.
      //if (confirm(msg))
      //{
        collab_util.openVtbe();
      //}
    }
  };
...
function hideComments( element, entryId )
{
  ...
  $('comments' + entryId + '_img').src = "/images/ci/ng/more_options_dark.gif";
                         ...
                         commentToFocusId )
{
  ...
  $('comments' + entryId + '_img').src = "/images/ci/ng/less_options_dark.gif";
